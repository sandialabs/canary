...
function(add_nvtest)
  ...
  if(PARSED_ARGS_COMMAND)
    ...
    string(APPEND CONTENT "nvtest.directives.set_attribute(program=\"${PROGRAM}\")\n")
    ...
    if(MPIEXEC_EXECUTABLE_OVERRIDE)
      ...
      string(APPEND CONTENT "    cmd = nvtest.Executable(\"${MPIEXEC_EXECUTABLE_OVERRIDE}\")\n")
      ...
      if(MPIEXEC_PREFLAGS_OVERRIDE)
        string(APPEND CONTENT "    preflags = \"${MPIEXEC_PREFLAGS_OVERRIDE}\" % {\"np\": 1}\n")
        string(APPEND CONTENT "    args.extend(preflags.split())\n")
      endif()
      string(APPEND CONTENT "    args.append(\"${PROGRAM}\")\n")
    else()
      string(APPEND CONTENT "    cmd = nvtest.Executable(\"${PROGRAM}\")\n")
    ...
    string(APPEND CONTENT "    p = argparse.ArgumentParser()\n")
    string(APPEND CONTENT "    p.add_argument(\"--execute-analysis-sections\", action=\"store_true\")\n")
    ...
    get_filename_component(F ${PARSED_ARGS_SCRIPT} NAME_WLE)
...
function(add_parallel_nvtest)
  ...
  if(MPIEXEC_EXECUTABLE_OVERRIDE)
    string(APPEND CONTENT "    mpi = nvtest.Executable(\"${MPIEXEC_EXECUTABLE_OVERRIDE}\")\n")
  elseif(MPIEXEC_EXECUTABLE)
    string(APPEND CONTENT "    mpi = nvtest.Executable(\"${MPIEXEC_EXECUTABLE}\")\n")
  else()
    string(APPEND CONTENT "    mpi = nvtest.Executable(\"mpiexec\")\n")
  endif()
  string(APPEND CONTENT "    args = []\n")
  if(MPIEXEC_NUMPROC_FLAG_OVERRIDE)
    string(APPEND CONTENT "    args.extend([\"${MPIEXEC_NUMPROC_FLAG_OVERRIDE}\", str(self.parameters.np)])\n")
  elseif(MPIEXEC_NUMPROC_FLAG)
    string(APPEND CONTENT "    args.extend([\"${MPIEXEC_NUMPROC_FLAG}\", str(self.parameters.np)])\n")
  else()
    string(APPEND CONTENT "    args.extend([\"-n\", str(self.parameters.np)])\n")
  endif()
  if(MPIEXEC_PREFLAGS_OVERRIDE)
    string(APPEND CONTENT "    preflags = \"${MPIEXEC_PREFLAGS_OVERRIDE}\" % {\"np\": self.parameters.np}\n")
    string(APPEND CONTENT "    args.extend(preflags.split())\n")
  elseif(MPIEXEC_PREFLAGS)
    string(APPEND CONTENT "    preflags = \"${MPIEXEC_PREFLAGS}\"\n")
    string(APPEND CONTENT "    args.extend(preflags.split())\n")
  endif()
  join_quoted("${COMMAND_LIST}" XTMP)
  string(APPEND CONTENT "    args.extend([${XTMP}])\n")
  string(APPEND CONTENT "    try:\n")
