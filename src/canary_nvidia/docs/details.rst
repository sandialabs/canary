...
The ``canary_nvidia`` extension provides the NVIDIA implementation of Canary's shared GPU backend
pattern. It uses ``nvidia-smi`` for device discovery and ``CUDA_VISIBLE_DEVICES`` for runtime
environment configuration. For AMD GPU support, see the ``canary_amd`` plugin.
...
- If ``CUDA_VISIBLE_DEVICES`` already set in job variables, the extension does not override it
...
- If no NVIDIA-compatible GPUs are allocated, no environment variable is set
