...
@canary.plugin.register(scope="test", stage="before_run")
def prelaunch(case: "TestCase", stage: str = "run") -> None:
    ...
    canary.filesystem.force_symlink(case.logfile(stage), f)


@canary.plugin.register(scope="test", stage="finish")
def write_execute_log(case: "TestCase") -> None:
    if not case.file_path.endswith(".vvt"):
        return
    f = os.path.join(case.working_directory, "execute.log")
    canary.filesystem.force_symlink(case.logfile(), f)
