...
class PluginHook:
    """Defines hooks into the canary execution.  User's can extend canary by subclassing this class
...
class Manager:
    ...
    def load_builtins(self) -> None:
        for resource in ir.files("_canary.plugins").iterdir():
            if not resource.name.startswith("_"):
                importlib.import_module(f".{resource.name}", "_canary.plugins")
    ...
    def load_from_entry_points(self, disable: list[str] | None = None):
        ...
        if self.entry_points_loaded:
            return
        entry_points = get_entry_points(group="canary.plugin")
        if entry_points:
...
def factory() -> Manager:
    ...
    if os.getenv("CANARY_LEVEL") == "0" and "CANARY_WORK_TREE" in os.environ:
        ...
        # was created
        file = os.path.join(os.environ["CANARY_WORK_TREE"], ".canary/objects/plugin")
