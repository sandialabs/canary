...
class SchemaOnlyOneAllowedError(SchemaError):
    """Error should be raised when an only_one Or key has multiple matching
    candidates"""
...
class Regex(object):
    ...
    def __init__(self, pattern_str, flags=0, error=None):
        ...
        flags_list = [
            Regex.NAMES[i] for i, f in enumerate("{0:09b}".format(flags)) if f != "0"
        ]  # Name for each bit
...
class Schema(object):
    ...
    def __init__(
        ...
        # Ask json_schema to create a definition for this schema and use it as
        # part of another
    ...
    def json_schema(self, schema_id, use_refs=False):
        ...
        :param use_refs: Enable reusing object references in the resulting JSON schema.
                         Schemas with references are harder to read by humans,
                         but are a lot smaller when there is a lot of reuse
        """
        ...
        def _json_schema(schema, is_main_schema=True, description=None, allow_reference=True):
            ...
            def _create_or_use_ref(return_dict):
                """If not already seen, return the provided part of the schema
                unchanged.  If already seen, give an id to the already seen dict
                and return a reference to the previous part of the schema instead.
                """
            ...
            def _to_json_type(value):
                """Attempt to convert a constant value (for "const" and
                "default") to a JSON serializable value"""
            ...
            if flavor == TYPE:
                ...
                if all(
                    ...
                    if len(any_of_values) == 1:
                        # Only one representable condition remains, do not put
                        # under oneOf
                ...
                if is_a_ref:
                    ...
                    for key in s:
                        ...
                        def _get_key_description(key):
                            """Get the description associated to a key (as
                            specified in a Literal object). Return None if not a
                            Literal"""
                        ...
                        def _get_key_name(key):
                            """Get the name of a key (as specified in a Literal
                            object). Return the key unchanged if not a
                            Literal"""
                            if isinstance(key, Optional):
                        ...
                        if isinstance(key_name, str):
                            ...
                            # This is less strict because we cannot enforce that
