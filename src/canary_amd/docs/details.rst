...
The ``canary_amd`` extension provides the AMD implementation of Canary's shared GPU backend
pattern. It uses ``amd-smi`` or ``rocm-smi`` for device discovery and sets
``HIP_VISIBLE_DEVICES``, ``ROCR_VISIBLE_DEVICES``, and ``CUDA_VISIBLE_DEVICES`` for runtime
environment configuration. For NVIDIA GPU support, see the ``canary_nvidia`` plugin.
...
- **Listing**: Enumerates AMD GPU devices via ``amd-smi list --json`` or ``rocm-smi`` output parsing
- **Environment**: Configures ``HIP_VISIBLE_DEVICES``, ``ROCR_VISIBLE_DEVICES``, and
  ``CUDA_VISIBLE_DEVICES`` for allocated GPUs
- **Integration**: Plugs into Canary's plugin hooks and resource pool framework

Detection Mechanism
-------------------

**Tools**: ``amd-smi`` (AMD System Management Interface) or ``rocm-smi`` (ROCm System Management
Interface)
...
- Rejects ``"UNKNOWN"`` vendor property (allows fallback to ``canary_nvidia``)
...
- If any of ``HIP_VISIBLE_DEVICES``, ``ROCR_VISIBLE_DEVICES``, or ``CUDA_VISIBLE_DEVICES``
  is already set in job variables, the extension does not override any of them
- User configuration takes precedence over automatic configuration

**Multi-Node Handling**:

- Deduplicates local GPU IDs while preserving order
- Important for multi-node allocations where nodes may share the same local IDs
- Uses ``dict.fromkeys()`` to remove duplicates while preserving insertion order

**Empty Allocation**:

- If no AMD-compatible GPUs are allocated, no environment variables are set
...
- **Solution**: Install AMD ROCm tools or use manual resource definition

**Enumeration Failure**:

- **Cause**: Both ``amd-smi`` and ``rocm-smi`` execution fail
...
9. **Vendor Specificity**: Does not claim UNKNOWN devices (allows ``canary_nvidia`` fallback)

Best Practices
--------------

1. **Install AMD Tools**: Ensure ``amd-smi`` or ``rocm-smi`` is installed and in PATH
2. **Prefer amd-smi**: Use ``amd-smi`` for more reliable UUID information
3. **Check Detection**: Verify backend detection with ``--verbose`` flag
4. **Test Environment**: Verify AMD environment variables in test setup
5. **Handle Missing Variables**: Provide fallback behavior when variables not set
6. **Respect User Overrides**: Don't override user-set environment variables
7. **Consider Multi-Node**: Handle duplicate local IDs appropriately
8. **Document Requirements**: Specify AMD ROCm driver and tool requirements
