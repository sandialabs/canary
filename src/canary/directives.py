...
def aggregate(
    *, when: WhenType | None = None, flag: str | None = None, script: str | None = None
    ...
    The composite base job will run after all other parameterized jobs and can be used to
    to ensure that the overall behavior of the parameterized jobs.  For example, a parameter
    may represent a step size and the composite base job will verify convergence as the step size
...
def cpus(arg: int, *, when: WhenType | None = None) -> None:
    ...
    * Use ``cpus(int)`` to set a single, fixed resource requirement for a job without
...
def gpus(arg: int, *, when: WhenType | None = None) -> None:
    ...
    * Use ``gpus(int)`` to set a single, fixed resource requirement for a job without
      creating additional test instantiations.
    * Test naming reflects only parameters introduced via `parameterize()`. Therefore,
...
@deprecated(
    "canary.directives.execution_path is deprecated; use canary_pyt.directives.execution_path"
)
