...
* Reworked :py:class:`_canary.resource_pool.ResourcePool` from a flat local inventory into a topology-aware resource model.
* Added explicit node modeling through :py:class:`_canary.resource_pool.rpool.Node`.
...
* Added :py:class:`_canary.resource_pool.manager.ResourceManager` to own the concrete resource pool for a configuration.
* :py:class:`_canary.config.Config` now constructs and manages resource pools lazily through the resource manager.
...
* Job and batch allocation state now stores full allocation objects containing:
  * ``metadata``;
  * ``resources``.
* :py:func:`_canary.job.Job.assign_resources` and :py:func:`_canary.job.Job.free_resources` now preserve the full allocation structure.
...
* Moved view management into a dedicated :py:mod:`_canary.view` module.
* Added :py:class:`_canary.view.ViewManager` for live, incremental view updates while jobs finish.
* Added manifest-backed views:
  * :py:class:`_canary.view.ViewManifest`;
  * :py:class:`_canary.view.ViewManifestEntry`;
...
* Added :py:func:`_canary.hookspec.canary_view_report` for view-level report generation.
...
* :py:class:`canary_hpc.batchspec.TestBatch` now stores and serializes full resource allocations.
...
* Added resource directives for ``.pyt`` tests:
  * :py:func:`canary_pyt.directives.cpus`;
  * :py:func:`canary_pyt.directives.gpus`;
  * :py:func:`canary_pyt.directives.nodes`.
* Resource directives are stored as meta-parameters so they affect scheduling without changing parameterized test names.
* Added support for PYT meta-parameters in generated :py:class:`_canary.ir.JobSpecIR` instances.
* Added :py:func:`canary_pyt.directives.artifact(..., save_on=...)` spelling for artifact save policy.
* Test instances now include spec attributes when converted through :py:mod:`_canary.testinst`.
...
* Added :py:class:`_canary.resource_pool.manager.ResourceManager`.
* Added :py:class:`_canary.resource_pool.rpool.Node`.
* Reworked :py:class:`_canary.resource_pool.rpool.ResourcePool` to manage one or more nodes.
...
* Added :py:class:`_canary.resource_pool.rpool.ResourceUnavailable` handling for unknown resources, unknown nodes, and heterogeneous per-node counts.
* Added :py:class:`_canary.resource_pool.rpool.EmptyResourcePoolError` behavior for empty pools.
...
* :py:class:`_canary.config.Config` now initializes a :py:class:`ResourceManager`.
...
* Added a dedicated :py:mod:`_canary.view` module containing:
  * :py:class:`ViewSettings`;
  * :py:class:`ResultsView`;
  * :py:class:`ViewManifest`;
  * :py:class:`ViewManager`;
  * :py:class:`ViewReportRequest`.
...
* Added public directive documentation for :py:func:`canary_pyt.directives.nodes`.
