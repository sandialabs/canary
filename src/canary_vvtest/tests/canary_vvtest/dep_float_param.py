...
"""Tests for VVT dependency-pattern substitution with floating-point parameter values.

Verifies that ``depends on : producer.my_var=${my_var}`` resolves correctly
when ``my_var`` is a float (e.g. ``.1`` / ``0.1``).
"""
...
def test_vvt_dep_float_param_substitution_resolves(tmp_path):
    """${my_var} in a VVT depends-on pattern resolves to the float-valued producer."""
    demo = """
