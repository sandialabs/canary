        ...
        for family in families:
            ...
            if analyze is not None:
                ...
                parent = JobSpecIR(
                    ...
                    file_path=Path(model.path),
                    family=family,
                    keywords=model.get_keywords(family, parameters=None, on_options=on_options),
                    timeout=model.get_timeout(family, parameters=None, on_options=on_options)
                    or -1.0,
                    baseline=[],
                    xstatus=model.get_xstatus(family, parameters=None, on_options=on_options),
                    preload=model.get_preload(family, parameters=None, on_options=on_options),
                    modules=[m.name for m in modules],
                    rcfiles=model.get_rcfiles(family, parameters=None, on_options=on_options),
                    owners=model.owners,
                    artifacts=model.get_artifacts(family, parameters=None, on_options=on_options),
                    exclusive=model.get_exclusive(family, parameters=None, on_options=on_options),
