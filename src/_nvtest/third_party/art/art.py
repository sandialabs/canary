...
def aprint(artname, number=1, space=1):
    ...
    try:
        ...
        result, artname = art(artname=artname, number=number, space=space, __detailed_return=True)
        print(result)
...
def tsave(
    ...
    try:
        ...
        result = text2art(
            text, font=font, decoration=decoration, chr_ignore=chr_ignore, sep=sep, space=space
        )
        file.write(result)
        file.close()
        if print_status:
...
def set_default(
    ...
    text2art.__defaults__ = (font, chr_ignore, decoration, sep, space, __detailed_return)
