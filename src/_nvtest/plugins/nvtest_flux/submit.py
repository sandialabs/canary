...
class Flux(HPCScheduler):
    ...
    def write_submission_script(self, batch: TestBatch, file: IO[Any]) -> None:
        ...
        qtime = self.qtime(batch, minutes=True) * self.timeoutx
        ...
        args.append(f"--output={batch.logfile()}")
        args.append(f"--error={batch.logfile()}")

        file.write(f"#!{self.shell}\n")
        for arg in args:
            file.write(f"#FLUX: {arg}\n")
    ...
    def submit_and_wait(self, batch: TestBatch) -> None:
        ...
        args = [self.exe, "batch", "--job-name", batch.name, script]
