...
Batch workspaces are created under the Canary session directory:

.. code-block:: text

   TestResults/sessions/<session>/batches/<batch-id-prefix>/

**Structure**:

- ``TestResults/sessions/<session>/``: Session directory
...
.. code-block:: text
   ...
   TestResults/sessions/<session>/batches/abc1234/
...
.. code-block:: console
   ...
   ls TestResults/sessions/<session>/batches/

   # Inspect specific workspace
   ls TestResults/sessions/<session>/batches/abc1234/

   # Read batch metadata
   cat TestResults/sessions/<session>/batches/abc1234/batch.lock

   # Check resource pool
   cat TestResults/sessions/<session>/batches/abc1234/resource_pool.json
...
.. code-block:: console
   ...
   cat TestResults/sessions/<session>/batches/abc1234/config.json

   # Review batch output
   cat TestResults/sessions/<session>/batches/abc1234/canary-out.txt

   # Check job results
   ls TestResults/sessions/<session>/batches/abc1234/results/
...
.. code-block:: console
   ...
   find TestResults/sessions/<session>/batches/ -type d

   # Check workspace contents
   tree TestResults/sessions/<session>/batches/abc1234/

   # Read specific files
   cat TestResults/sessions/<session>/batches/abc1234/batch.lock | jq .
...
.. code-block:: console
   ...
   rm -rf TestResults/sessions/<session>/batches/abc1234/

   # Remove all workspaces
   rm -rf TestResults/sessions/<session>/batches/*

   # Remove HPC cache
   rm -rf TestResults/sessions/<session>/
