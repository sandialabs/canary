...
from canary_hpc.partitioning import ONE_PER_BUCKET
...
def test_batch_n(generate_files):
    ...
    batches = batching.batch_testcases(cases=cases, batchspec=spec)
    assert len(batches) == 5
    assert sum(len(_) for _ in batches) == num_cases
    spec = {"count": ONE_PER_BUCKET, "duration": None, "nodes": "any", "layout": "flat"}
    batches = batching.batch_testcases(cases=cases, batchspec=spec)
...
def test_batch_t(generate_files):
    ...
    batches = batching.batch_testcases(cases=cases, batchspec=spec)  # 5x long test case duration
    assert sum(len(_) for _ in batches) == num_cases
