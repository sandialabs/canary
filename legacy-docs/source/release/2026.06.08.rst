...
* Introduced :py:class:`_canary.job.BaseJob` and standardized queue/executor interactions around a job-phase lifecycle.
* Backward compatibility is preserved: :py:class:`canary.TestCase`` remains available as an alias of :py:class:`canary.Job`, so most existing client code does not need to change.

New spec pipeline and dependency resolution
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* Added :py:class:`_canary.ir.JobSpecIR` for generator outputs that contain unresolved dependency selectors.
* Added :py:class:`_canary.jobspec.JobSpec` as the resolved, runnable spec format.
* Added :py:mod:`_canary.resolve_dependency` to resolve dependency patterns/selectors into concrete spec dependencies.

Status system refactor
^^^^^^^^^^^^^^^^^^^^^^

* Replaced legacy status tuple concepts with:
  * :py:class:`_canary.status.Category` (PASS/FAIL/SKIP/CANCEL/NONE)
  * :py:class:`_canary.status.Outcome` (SUCCESS/FAILED/TIMEOUT/…)
  * Updated :py:class:`_canary.status.Status` API (``category/outcome/reason/code``).
...
* Added :py:class:`_canary.job.Job`, :py:class:`_canary.job.BaseJob`, :py:class:`_canary.job.JobPhase`, :py:class:`_canary.job.JobState`.
...
* Added :py:class:`_canary.ir.JobSpecIR` and :py:func:`_canary.resolve_dependency.resolve`.
...
* Implemented enum-based status model in :py:class:`_canary.status`.
...
* YAML example generator updated:
  ...
  * template substitution via :py:class:`string.Template`.
