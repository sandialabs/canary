...
def canary_examples_dir() -> Path:
    ...
    """
    return Path(str(resources.files("canary").joinpath("examples")))
...
def _execute(invocation: Invocation, *, returncode: int, anyreturncode: bool) -> ExecutionRecord:
...
def _execute_in_directory(
    invocation: Invocation, *, temp_root: Path, returncode: int, anyreturncode: bool
) -> ExecutionRecord:
...
def _record_is_usable(
    ...
    *,
    ...
    for script in record.script:
        if script.returncode is None:
            return False
        if not anyreturncode and script.returncode != expected_returncode:
            return False
...
def _failure_reason(node: doc_run_output, record: ExecutionRecord) -> str | None:
    ...
    for script in record.script:
        ...
        if not node["anyreturncode"] and command_rc != node["returncode"]:
            return (
                f"script command {script.command!r} returned unexpected code "
                f"{command_rc} != {node['returncode']}"
            )
    ...
    for node in traverse(doctree, doc_run_output):
        ...
        try:
            record = cache.get(
                ...
                returncode=int(node["returncode"]),
