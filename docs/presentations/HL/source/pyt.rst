...
.. code-block:: python
    ...
    canary.directives.parameterize("type", [0, 1])


    def test() -> int:
        case = canary.get_instance()
        job = alegra.AlegraJob(case.family, nproc=case.cpus, dimension=3)
        job.preprocess_input_file(TYPE=case.parameters.type)
        job.submit()
        seacas.exodiff(job.name)
        return 0
