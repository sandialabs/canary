...
In addition to the command line, configuration variables can be explicitly set ``yaml`` formatted files.  ``canary`` first looks for the "global" configuration in ``~/.config/canary/config.yaml``.  You can modify the location of this file by setting the ``CANARY_CONFIG_HOME`` or ``XDG_CONFIG_HOME`` environment variables, in which case the configuration will be found in ``$CANARY_CONFIG_HOME/config.yaml`` or ``$XDG_CONFIG_HOME/canary/config.yaml``.  Setting ``CANARY_CONFIG_HOME=null`` will cause ``canary`` to ignore this configuration scope.
