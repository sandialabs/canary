...
``nvtest`` finds and runs tests defined in Python files having a ``.pyt`` extension.  The files are executed and return code captured.  A return code of ``0`` are considered to be ``success`` while any other return code indicates a failure.
