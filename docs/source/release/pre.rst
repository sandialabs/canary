canary 0.0.0 Changelog
======================

.. contents::

Changelog of initial, pre-release, versions of ``canary``

October 2024
------------

- Move main repo to https://cee-gitlab.sandia.gov/ascic-test-infra/canary
- Move all plugins into ``_canary/plugins``.  This includes commands, reporters, runners, etc.

September 2024
--------------

- Minimum Python version is 3.10
- Removed ``canary show`` - use ``canary describe``
...
- Send CPU and GPU ids to tests.  These are the canary internal IDs, not necessarily the hardware IDs
...
- Added ``canary log`` and ``canary location`` subcommands
...
- Added ``-l scope:resource=value`` flag to ``canary run``
...
- Add ``canary -p`` plugin flag and move builtin plugins to ``canary/plugins``
...
- Added ``-p`` parameter filter flag to ``canary run``
