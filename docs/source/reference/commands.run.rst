...
.. code-block:: console
   ...
   resource control:
     ...
     --timeout type=T      Set the timeout for **type** (accepts Go's duration format, eg, 40s, 1h20m, 2h,
                           ...
                           Otherwise, a timeout of T is applied to tests having keyword **type**. Eg,
                           **--timeout fast=2** would apply a timeout of 2 seconds to all tests having the
                           'fast' keyword; common types are fast, long, default, and ctest.
   ...
   canary hpc:
     ...
     --hpc-submit-arg, --scheduler-args ARGS
                           ...
                           options=ARGS]
     --hpc-batch-spec SPEC
                           Comma separated list of options to partition jobs into batches. See canary batch
                           help --spec for help on batch specification syntax [alias: -b spec=SPEC]
     ...
     --hpc-batch-exact-estimate
                           After forming batches with cheap schedule estimates, run an exact scalar
                           scheduler simulation once per final batch to refine the stored runtime estimate.
                           This is slower for very large suites.
