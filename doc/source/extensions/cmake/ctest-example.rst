...
Example Without doc-run
~~~~~~~~~~~~~~~~~~~~~~~

This example cannot use ``doc-run`` because:

1. **Resource Requirements**: The resource group tests require GPU resources that may not be available
2. **Environment Setup**: Needs proper CTest and CMake environment
3. **External Dependencies**: Depends on system configuration

Note that the basic ``ctest_test`` (running ``ls /``) does not require special resources, but the resource group tests do require GPU-like resource groups to be configured.

For a deterministic ``doc-run`` example, see the simple CTest examples in the Canary tests.
